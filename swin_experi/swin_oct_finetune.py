...
class Engine:
    ...
    def train(model, train_dataloader, test_dataloader, optimizer, loss_fn, epochs, device, save_dir="model_checkpoints"):
        ...
        for epoch in range(epochs):
            ...
            # Save the model
            model_path = f"{save_dir}/swinv2b_torch_oct{train_loss:.4f}_{train_accuracy:.4f}_{test_loss:.4f}_{test_accuracy:.4f}.pth"
...
pretrained_swin = torch.load('model_checkpoints/swinv2b_torch_xray0.2680_0.8854_0.2336_0.9016.pth') # Load the pretrained model on x-ray
if isinstance(pretrained_swin, torch.nn.DataParallel):
           pretrained_swin = pretrained_swin.module

for param in pretrained_swin.parameters(): #freeze model
    param.requires_grad = False

n_inputs = pretrained_swin.head[-1].in_features
pretrained_swin.head[-1] = torch.nn.Linear(n_inputs, 512)  # Adjust the number of output features of the last linear layer

n_inputs = 512  # Updated input size for the new linear layer
pretrained_swin.head = torch.nn.Sequential(
    pretrained_swin.head,
    torch.nn.ReLU(),
...
set_seeds()
# Training loop
pretrained_vit_results = engine.train(model=pretrained_swin,
