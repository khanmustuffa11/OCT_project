...
with strategy.scope():
    ...
    del base_model
    model.load_weights("model_convnext031--0.382818--0.985649--0.420023--0.965179.h5", by_name=True, skip_mismatch = True)

    model.summary()
    model.compile(optimizer=adm, loss=loss,metrics=['accuracy'])

mcp_save = ModelCheckpoint('model_convnext_finetune{epoch:03d}--{loss:03f}--{accuracy:03f}--{val_loss:03f}--{val_accuracy:03f}.h5', verbose=1, monitor='val_loss',save_best_only=True, mode='min')
model.fit(
