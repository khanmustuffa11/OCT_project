...
with strategy.scope():
    ...
    conv_output_attention = MultiHeadAttention(num_heads=8, key_dim=256)(conv_base.output, conv_base.output) 
    residual = layers.Multiply()([conv_base.output, conv_output_attention])   
    pooled = layers.GlobalAveragePooling2D()(residual)
    x = Dropout(0.25)(pooled)
...
mcp_save = ModelCheckpoint('mla_8class_{epoch:03d}--{loss:03f}--{accuracy:03f}--{val_loss:03f}--{val_accuracy:03f}.h5', verbose=1, monitor='val_loss',save_best_only=True, mode='min')
model.fit(
