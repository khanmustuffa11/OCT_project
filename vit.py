...
DR_LABELS = []
num_classes = len(DR_LABELS+DISEASE_LABELS)
...
class Engine:
    ...
    def train(model, train_dataloader, test_dataloader, optimizer, loss_fn, epochs, device, save_dir="model_checkpoints"):
        ...
        for epoch in range(epochs):
            ...
            # Save the model
            model_path = f"{save_dir}/vit_torch_trail2{train_loss:.4f}_{train_accuracy:.4f}_{test_loss:.4f}_{test_accuracy:.4f}.pth"
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = self.data_frame.iloc[idx][DR_LABELS + DISEASE_LABELS].values
