...
df_train, df_test = pd.read_csv('oct_final_train_vit.csv'), pd.read_csv('oct_final_val_vit.csv')
...
class Engine:
    ...
    def train(model, train_dataloader, test_dataloader, optimizer, loss_fn, epochs, device, save_dir="model_checkpoints"):
        ...
        for epoch in range(epochs):
            ...
            # Save the model
            model_path = f"{save_dir}/vit_oct_xray{train_loss:.4f}_{train_accuracy:.4f}_{test_loss:.4f}_{test_accuracy:.4f}.pth"
...
pretrained_vit = torch.load('model_checkpoints/vit_torch_xray0.3903_0.9243_0.4058_0.9182.pth')

pretrained_vit.module.heads = torch.nn.Linear(in_features=pretrained_vit.module.heads.in_features, out_features=8)
